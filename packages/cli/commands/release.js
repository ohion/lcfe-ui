...
const outputDir = path.resolve(__dirname, '../../lcfe-ui/build');

const packageJson = require('../../lcfe-ui/package.json');
...
exports.release = async ({ version }) => {
  ...
  shell.cp('-R', path.resolve(__dirname, '../../lcfe-ui/README.md'), outputDir);
  shell.cd(outputDir);
  shell.mkdir('-p', 'theme');
  shell.cp(
    '-R',
    path.resolve(__dirname, '../../lcfe-ui/ui/theme/theme.scss'),
    path.resolve(outputDir, 'theme')
  );
  shell.cp(
    '-R',
    path.resolve(__dirname, '../../lcfe-ui/ui/theme/darkTheme.css'),
    path.resolve(outputDir, 'theme')
  );
